...
header = {
    "Authorization": "Bearer " + tmconfig.xdr_token,
...
def post(url_path, query_params, body):
    ...
    if (200 == r.status_code) and (
        ...
        return r.json()
    raise RuntimeError(
        f"Request unsuccessful (POST {url_path}):" f" {r.status_code} {r.text}"
    )
...
def delete(url_path, query_params):
    ...
    if "application/json" in r.headers.get("Content-Type", ""):
        return r.json()
    raise RuntimeError(
        f"Request unsuccessful (DELETE {url_path}):" f" {r.status_code} {r.text}"
    )
...
def put(url_path, query_params, body):
    ...
    if (200 == r.status_code) and (
        ...
        return r.json()
    raise RuntimeError(
        f"Request unsuccessful (PUT {url_path}):" f" {r.status_code} {r.text}"
    )
...
def setPassword(email, password):
    ...
    url_path = url_path.format(**{"email": email})
    query_params = {}
    data = {"type": 0, "password": password, "token": tmconfig.xdr_token}
    body = json.dumps(data)
