...
def fetch_observed_attack_techniques(v1, start, end):
    ...
    while True:
        ...
        print(
            f"Observed Attack Technique events({offset} {offset+size}):"
            f" {len(gotten)}"
        )
    ...
    for d in docs["observed_techniques"]:
        ...
        for f in d.get("filters", []):
            for obj in f.get("highlightedObjects", []):
                if ("text" == obj["type"]) and (not isinstance(obj["value"], str)):
...
def index_data_to_es(es, docs):
    def index_actions(name, data):
        for source in data:
            yield {"_index": name, "_op_type": "index", "_source": source}
...
def pull_v1_data_to_es(v1, es, start, end, index_prefix, include_detections):
    ...
    docs["observed_techniques"] = fetch_observed_attack_techniques(v1, start, end)
...
def main(
    ...
    if "https" == urllib.parse.urlparse(es_url).scheme:
        ssl_context = ssl.create_default_context(cafile=es_cafile, capath=es_capath)
    if ssl_context and (es_certfile or es_keyfile):
        ssl_context.load_cert_chain(certfile=es_certfile, keyfile=es_keyfile)
    v1 = TmV1Client(v1_token, v1_url)
    es = elasticsearch.Elasticsearch(host, http_auth=http_auth, ssl_context=ssl_context)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            f"Example: python {os.path.basename(__file__)} "
    ...
    parser.add_argument(
        ...
        "--start",
        help=(
            "Timestamp in ISO 8601 format that indicates the start of"
    ...
    parser.add_argument(
        ...
        "--end",
        help=(
            "Timestamp in ISO 8601 format that indicates the end of the data"
    ...
    parser.add_argument(
        ...
        default=5,
        help=(
            "Number of days before the end time of the data retrieval"
    ...
    parser.add_argument(
        ...
        "--v1-token",
        default=V1_TOKEN,
        help=("Authentication token of your Trend Micro Vision One" " user account"),
    )
    parser.add_argument(
        "-r",
        "--v1-url",
        ...
        help=(
            "URL of the Trend Micro Vision One server for your region."
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            'Parameter that searches the "Detections" data via API'
    ...
    parser.add_argument(
        ...
        "--es-url",
        default=ES_URL,
        help=("URL of the Elasticsearch server. The default value is" f' "{ES_URL}"'),
    )
    parser.add_argument(
        "-p",
        "--prefix",
        default=ES_INDEX_PREFIX,
        help=(
            "Prefix of indices in Elasticsearch. The default value is"
            f' "{ES_INDEX_PREFIX}"'
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=ES_PASSWORD,
        help=(
            "Password for Elasticsearch authentication. If you specify"
    ...
    parser.add_argument(
        ...
        default=ES_CAFILE,
        help=(
            "Path to a file containing a CA certificate for TLS or SSL"
    ...
    parser.add_argument(
        ...
        default=ES_CAPATH,
        help=(
            "Path to a directory containing CA certificates for TLS or SSL"
            " connection to Elasticsearch"
        ),
    )
    parser.add_argument(
        ...
        default=ES_CERTFILE,
        help=(
            "Path to a file containing a certificate for TLS or SSL"
            " connection to Elasticsearch"
        ),
    )
    parser.add_argument(
        ...
        default=ES_KEYFILE,
        help=(
            "Path to a file containing a private key for TLS or SSL"
            " connection to Elasticsearch"
        ),
    )
