...
def main():
    try:
        ...
        par = argparse.ArgumentParser(
            description="Trend Micro Vision One File Collector",
            formatter_class=argparse.RawTextHelpFormatter,
        )
        par.add_argument("-e", "--endpointName", help="EndpointName", required=False)
        par.add_argument(
            "-f", "--fileFullPatch", help="File with full path", required=False
        )
        par.add_argument(
            "-d", "--description", help="Description", default="", required=False
        )
        par.add_argument(
            "-i",
            "--interval",
            help="Interval (min) to verify if the file is ready to download",
            type=int,
            default=5,
            required=False,
        )
        par.add_argument(
            "-r", "--retrieve", help="retrieve a file via task number", required=False
        )
        args = par.parse_args()
        ...
        # tmconfig.zone
        x = TMVisionOne.XDR(
            "us", tmconfig.xdr_token, "Trend Micro Vision One CollectFile.py sample"
        )
...
if __name__ == "__main__":
    ...
    print("=================================================")
    print(
        "To send the collect command, use these parameters and keep track of the task number"
    )
    print(
        """V1CollectFile.py -e CA-DAVEG-L -f "c:\data\winword.exe" -d "collect test file" """
    )
    print(
    ...
    print(
        "Note: make sure tmconfig.py is next to this script and that you configured the zone and xdr_token variables"
    )
    print("=================================================")
    main()
