...
class TmV1Client:
    ...
    def get_workbench_histories(
        self, start, end, status_list=None, offset=None, size=None
    ):
    ...
    def query_endpoint_info(self, computer_id):
        return self.post(
            "/v2.0/xdr/eiqs/query/endpointInfo", json={"computerId": computer_id}
        )["result"]

    def isolate_endpoint(self, computer_id):
        return self.post("/v2.0/xdr/response/isolate", json={"computerId": computer_id})
    ...
    def quarantine_message(self, mail_message_id, mailbox, mail_message_delivery_time):
        return self.post(
...
def create_message_query(entity, indicators):
    ...
    for indicator in indicators:
        if (
            indicator["id"] in entity["relatedIndicators"]
        ...
        for entity in impact_scope:
            ...
            if entity_type == "host":
                ...
                if endpoint_info["productCode"] == "sao":
                    ...
                    msg = (
                        "Nothing has done with this endpoint "
                ...
                mail_logs = v1.search_message_activities(start, end, query)["data"][
                    "logs"
                ]
                if mail_logs:
                    for mail in mail_logs:
                        msg = (
                            "A message quarantined."
                        ...
                        v1.add_workbench_notes(wb_id, msg)
                        quarantined_messages.append(mail["mail_message_subject"])
            print(
                "\rAffected entities that received response actions: "
