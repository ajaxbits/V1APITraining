...
from datetime import (
    ...
    timedelta,
    timezone,
)  # to calculate date and time range of the search


def optionparse():
    """Argument Parser."""
    opts = argparse.ArgumentParser(
        description="V1 Search Script", formatter_class=argparse.RawTextHelpFormatter
    )
    ...
    opts.add_argument(
        "-l", "--datalake", help="Data Lake to query: edr, msg, net, det", default="edr"
    )
    opts.add_argument("-q", "--query", help="XDR search query")
    opts.add_argument(
        "-t", "--output_type", default="json", help="output to json or csv"
    )
...
def main(args):
    ...
    try:
        if args.query:
            ...
            # change your region
            x = TMVisionOne.XDR(
                "us", tmconfig.xdr_token, "Trend Micro Vision One Search Script Sample"
            )
            my_date = datetime.now() - timedelta(days=int(args.days))
            istart = x.convert_DateToEpoc(str(my_date.replace(microsecond=0)))
            iEnd = x.convert_DateToEpoc(
                str(datetime.today().replace(microsecond=0) + timedelta(days=1))
            )
            ...
            print(
                "No query supplied, example -q 'hostName:globalnetworkissues.com OR objectIps:(204.188.205.176 OR 5.252.177.25)'"
            )
